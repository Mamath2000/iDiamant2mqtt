...
class MQTTClient {
  ...
  async connect() {
    return new Promise((resolve, reject) => {
      try {
        ...
        options.will = {
          topic: `${this.config.MQTT_TOPIC_PREFIX}/bridge/state`,
          payload: 'offline',
          qos: 1,
          retain: true
        };
        ...
        this.client.on('connect', () => {
          ...
          this.publish(`${this.config.MQTT_TOPIC_PREFIX}/bridge/state`, 'online', { retain: true });
  ...
  extractDeviceIdFromTopic(topic) {
    // Extraction de l'ID du dispositif depuis le topic
    const match = topic.match(/\/([^\/]+)\/set$/);
